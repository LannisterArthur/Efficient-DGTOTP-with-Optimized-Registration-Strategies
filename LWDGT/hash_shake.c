...
void hash_message_myots(unsigned char *digest, uint64_t *tree, uint32_t *leaf_idx,
                        const unsigned char *R, const unsigned char *pk,
                        const unsigned char *m, unsigned long long mlen,
                        const spx_ctx *ctx)
{
    (void)ctx;
#define SPX_TREE_BITS (MYOTS_TREE_HEIGHT * (MYOTS_D - 1))
#define SPX_TREE_BYTES ((SPX_TREE_BITS + 7) / 8)
#define SPX_LEAF_BITS MYOTS_TREE_HEIGHT
#define SPX_LEAF_BYTES ((SPX_LEAF_BITS + 7) / 8)
