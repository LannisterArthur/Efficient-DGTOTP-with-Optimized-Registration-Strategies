...
#define MAX_HASH_TRIALS_WOTS (1 << (20))
    ...
    *counter_out = 0;
    if (idx_leaf != to_sign)
    {
        /*Set thash address for custom hash*/
        uint32_t *pk_addr = info.pk_addr;
        set_keypair_addr_myots(pk_addr, idx_leaf);
        set_type(pk_addr, SPX_ADDR_TYPE_COMPRESS_WOTS);
        thash_init_bitmask(bitmask, 1, ctx, pk_addr);
...
void merkle_gen_root_myots(unsigned char *root, const spx_ctx *ctx)
{
    /* We do not need the auth path in key generation, but it simplifies the
       code to have just one treehash routine that computes both root and path
       in one function. */
    unsigned char auth_path[MYOTS_TREE_HEIGHT * SPX_N + SPX_WOTS_BYTES];
    uint32_t top_tree_addr[8] = {0};
    uint32_t wots_addr[8] = {0};
    uint32_t counter;

    set_layer_addr(top_tree_addr, MYOTS_D - 1);
    set_layer_addr(wots_addr, MYOTS_D - 1);

    merkle_sign_myots(auth_path, root, ctx,
                      wots_addr, top_tree_addr,
                      ~0 /* ~0 means "don't bother generating an auth path */,
                      &counter);
}